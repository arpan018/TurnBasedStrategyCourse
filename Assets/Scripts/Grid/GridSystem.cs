...
public class GridSystem
{
    ...
    public void CreateGridObjects(Transform gridVisualPrefab)
    {
        for (int x = 0; x < width; x++)
        {
            for (int z = 0; z < height; z++)
            {
                GridPosition gridPosition = new(x, z);
                var debugObject = GameObject.Instantiate(gridVisualPrefab, GetWorldPosition(gridPosition), Quaternion.identity);
